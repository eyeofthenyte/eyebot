...
class ConfigService:
    def __init__(self, configPath: str, logger: LogService = None) -> None:
        ...
        self.logger = logger
        self.config = {}
        stem = os.path.splitext(os.path.basename(configPath))[0]
        self.backup_prefix = "backup-" if stem == "config" else f"{stem}-backup-"
    ...
    def _ensure_backup(self):
        ...
        backup_file = os.path.join(backup_dir, f"{self.backup_prefix}{today}.bak")

        # Remove any existing backups (only 1 kept at a time)
        for file in os.listdir(backup_dir):
            if file.startswith(self.backup_prefix) and file.endswith(".bak") and file != os.path.basename(backup_file):
                try:
    ...
    def _recover_from_backup(self) -> bool:
        ...
        backups = [
            ...
            if f.startswith(self.backup_prefix) and f.endswith(".bak")
        ]
        if not backups:
            if self.logger:
