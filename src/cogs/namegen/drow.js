const nameParts = {
    ...
    nmSRf: ["Alean", "Ale", "Arab", "Arken", "Auvry", "Baen", "Barri", "Cladd", "Desp", "De", "Do'", "Eils", "Everh", "Fre", "Gode", "Helvi", "Hla", "Hun'", "Ken", "Kil", "Mae", "Mel", "My", "Noqu", "Orly", "Ouss", "Rilyn", "Teken'", "Tor", "Zau"],
    nmSRl: ["afin", "ana", "ani", "ar", "arn", "ate", "ath", "duis", "ervs", "ep", "ett", "ghym", "iryn", "lyl", "mtor", "ndar", "neld", "rae", "rahel", "rret", "sek", "th", "tlar", "t'tar", "tyl", "und", "urden", "val", "viir", "zynge"],
    nmSFf: ["A'Dar", "Ab", "Abb", "Arg", "Baenre", "Belt", "Blaer", "Blund", "Chaulss", "Cob", "Col", "Cormr", "D", "Daev", "Dal", "Dhalm", "Dhunn", "Dil", "Din", "Dryaal", "Duskr", "Elpr", "Faert", "Fil", "Gall", "Glann", "Glaur", "H", "Helv", "Hunzr", "Hyl", "Ich", "Ilal", "Ill", "Jhal", "Juszt", "Keteer", "Khal", "Kront't", "Lhal", "Luelt", "Mizzr", "Mlezz", "N", "Nir", "Ol", "Omr", "Phil", "Quav", "Rhomd", "Rrost", "Seer", "Ss", "T", "T'", "T's", "Tanor'Th", "Tel", "Tlin'", "Tlint", "Ul", "Vr", "Vramm", "Waegl", "Xilt", "Yaunt", "Yril'Lys", "Zaphr", "Zauv", "Zol"],
    nmSFl: ["abar", "abban", "ach", "aeir", "ael", "aen", "aer", "aerth", "agh", "agon", "al", "ala", "ambra", "ane", "ar", "arn", "arr", "arran", "aryd", "ass", "ath", "aulur", "avar", "azza", "ear", "ein", "enna", "esz", "eztice", "ifar", "iir", "iirn", "iiryn", "in", "inath", "inn", "iom", "ion'lyr", "iriy", "is", "istyn", "ith", "iwin", "oara", "oavae", "ond", "onrae", "orel", "orgh", "oryn", "orzza", "ossz", "uae", "uan", "uil", "uin", "une", "ykur", "yl", "ylan", "ym", "yn", "yr", "yrr", "yth"],
};
...
function generateName(type = "m", subrace = null) {
    ...
    let sr = nameData[subrace] || nameData[subrace.toLowerCase()] || nameData["drow"];
    if (!sr) return "";
    ...
    if (g === "male") {
        if (i < 4) {
            const rnd = Math.floor(Math.random() * nameParts.nmMRf.length);
            const rnd2 = Math.floor(Math.random() * nameParts.nmMRl.length);
            first = nameParts.nmMRf[rnd] + nameParts.nmMRl[rnd2];
        } else if (i < 8) {
            const rnd = Math.floor(Math.random() * nameParts.nmMFf.length);
            const rnd2 = Math.floor(Math.random() * nameParts.nmMFl.length);
            first = nameParts.nmMFf[rnd] + nameParts.nmMFl[rnd2];
        } else {
            const rnd = Math.floor(Math.random() * nameParts.nmM.length);
            first = nameParts.nmM[rnd];
        }
    } else {
        if (i < 4) {
            const rnd = Math.floor(Math.random() * nameParts.nmFRf.length);
            const rnd2 = Math.floor(Math.random() * nameParts.nmFRl.length);
            first = nameParts.nmFRf[rnd] + nameParts.nmFRl[rnd2];
        } else if (i < 8) {
            const rnd = Math.floor(Math.random() * nameParts.nmFFf.length);
            const rnd2 = Math.floor(Math.random() * nameParts.nmFFl.length);
            first = nameParts.nmFFf[rnd] + nameParts.nmFFl[rnd2];
        } else {
            const rnd = Math.floor(Math.random() * nameParts.nmF.length);
            first = nameParts.nmF[rnd];
        }
    }

    if (i < 4) {
        const rnd3 = Math.floor(Math.random() * nameParts.nmSRf.length);
        const rnd4 = Math.floor(Math.random() * nameParts.nmSRl.length);
        last = nameParts.nmSRf[rnd3] + nameParts.nmSRl[rnd4];
    } else if (i < 8) {
        const rnd3 = Math.floor(Math.random() * nameParts.nmSFf.length);
        const rnd4 = Math.floor(Math.random() * nameParts.nmSFl.length);
        last = nameParts.nmSFf[rnd3] + nameParts.nmSFl[rnd4];
    } else {
        const rnd3 = Math.floor(Math.random() * nameParts.nmS.length);
        last = nameParts.nmS[rnd3];
    }
