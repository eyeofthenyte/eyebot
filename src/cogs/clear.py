...
class Clear(commands.Cog):
    ...
    def load_config(self):
        if self.platform_config_service is not None:
            self.mod_channel_name = self.platform_config_service.platform(
                "discord"
            ).get("mod_channel_name", "mod-logs")
            return self.platform_config_service.discord_guilds()

        default = {
            "mod_channels": {},
            "mod_channel_name": "mod-logs",
            "timers": {}
        }
        ...
        self.mod_channel_name = legacy.get("mod_channel_name", "mod-logs")
        guilds = {}
        for guild_id in set(legacy.get("mod_channels", {})) | set(legacy.get("timers", {})):
            guilds[guild_id] = {
                "mod_channel": legacy.get("mod_channels", {}).get(guild_id, "UNSET"),
                "timers": legacy.get("timers", {}).get(guild_id, {}),
            }
        return guilds
    ...
    def save_config(self):
        ...
        legacy = {
            "mod_channels": {
                guild_id: data.get("mod_channel", "UNSET")
                for guild_id, data in self.config.items()
            },
            "mod_channel_name": self.mod_channel_name,
            "timers": {
                guild_id: data.get("timers", {})
                for guild_id, data in self.config.items()
            },
        }
        with open(CONFIG_PATH, "w") as f:
    ...
    def guild_config(self, guild_id):
        config = self.config.setdefault(str(guild_id), {})
        config.setdefault("mod_channel", "UNSET")
    ...
    async def clear(self, ctx, amount: int = 100):
        '''
        :wastebasket:  **__Clear__**

        Usage:
        `!clear #`
        `# = number of messages you want to clear`
        Will delete the number of messeges indicated or 100 if no number is given.
        Pompts will appear if you attempte to delete more than a 1 message at a time.
        '''
    ...
    async def settimer(self, ctx, interval: int, duration: int = None):
        ...
        if interval == 0:
            # Disable timer
            self.guild_config(guild_id)["timers"].pop(channel_id, None)
            self.save_config()
        ...
        # Otherwise, set timer...
        timers = self.guild_config(guild_id)["timers"]
        expires_at = None
        if duration:
    ...
    async def timer_loop(self):
        ...
        for guild_id, guild_config in self.config.items():
            channels = guild_config.get("timers", {})
            ...
            for channel_id, data in list(channels.items()):
                ...
                if expires and now >= datetime.fromisoformat(expires):
                    ...
                    mod_channel_id = guild_config.get("mod_channel")
