...
COMPONENTS_SHEET_KEY = '1wkaF--4DqTComUfX1dYbjhkRimR2raYbJm3IEa1CTq0'
...
class Components(commands.Cog):
    ...
    async def _flora(self, ctx, *, select):
        try:
            ...
            self.bot.logger.error(f'Ingredient data unavailable: {error}')
            await ctx.send("❌ Ingredient data is temporarily unavailable.")
            return
    ...
    async def potion(self, ctx, *, select):
        try:
            ...
            self.bot.logger.error(f'Potion data unavailable: {error}')
            await ctx.send("❌ Potion data is temporarily unavailable.")
            return
    ...
    async def poison(self, ctx, *, select):
        try:
            ...
            self.bot.logger.error(f'Poison data unavailable: {error}')
            await ctx.send("❌ Poison data is temporarily unavailable.")
            return
