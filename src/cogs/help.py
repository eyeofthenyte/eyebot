...
class Help (commands.Cog):
        ...
        def get_commands():
            ...
            for filename in os.listdir(currDir):
                if filename.endswith('.py'):
                    all_commands.append(filename[:-3])
            return all_commands

        commands = get_commands()

        if (ctx.author==ctx.guild.owner or ctx.author.has_permissions(administrator=True)):
            ...
            embed.add_field(name=f'**__Commands__**', value="\n".join(commands), inline=False)
    ...
    #async def _help(self, ctx, *):
        #file = discord.File('./eyebot/images/system/warning.png', filename='warning.png')
        #embed = discord.Embed(color=0x019cd0)
        #embed.set_author(name=f'All Commands', icon_url='attachment://prohibited.png')
